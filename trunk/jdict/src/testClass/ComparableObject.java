...
public class ComparableObject implements Comparable<Object> {
	...
	System.out.println("comparing this.myint " + this.myInt
		+ " against inObject.myint " + obj.myInt);
	...
	if (this.myInt == ((ComparableObject) inObject).myInt)
	    return true;
	...
	System.out.println("-----------");
	map.put(c, 2); // if i try to put the same key it overwrites the
			// previous
	System.out.println("-----------");
	map.put(d, 3);
	System.out.println("-----------");
	...
	// if(map.containsKey(b)) // if containsKey returns true update the
	// value
	// map.put(b, 3);

	/*
	 * System.out.println("contains key?? "+map.containsKey(b));
	 * //containsKey calls the method compareTo
	 * 
	...
	while (itr.hasNext()) {
	    ...
	    System.out.println("key " + key.myInt + " value " + map.get(key)); // containsKey
										// calls
										// the
										// method
										// compareTo
