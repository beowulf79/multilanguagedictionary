...
public final class SleepyVerbsDatabaseLoader {
	...
	private SleepyVerbsDatabaseWriter writer;
	...
	private void iterateLanguages() throws DatabaseException,
		...
		try {
			...
			HashMap<String,LanguageConfigurationClassDescriptor> ldesc = LanguagesConfiguration.getNuovo();
			...
			for (Iterator<String> it = ldesc.keySet().iterator(); it.hasNext();) {
				...
				//check if the language is enabled , if not skip to the next language
				if (!sub.getIsEnabled()) {
			...
			//	translations = (List<String>) sub.getList("translation");
				...
				writer = new SleepyVerbsDatabaseWriter(factory
						.getDatabase(nickname+type));
				writer.setDataBinding(SleepyBinding
						.getDataBinding());
				...
				class_array = (SearchableObject[]) Array.newInstance(IClass, rowsNumber);

				for (int i = 0; i < rowsNumber; i++) {
					class_array[i] = (SearchableObject) IConstructor.newInstance();
				}
	...
	private void iterateLabels(String _language, String _type) throws LabelNotFoundException, BiffException,
			IOException, KeyNotFoundException {

		try {

			LanguageConfigurationClassDescriptor sub = LanguagesConfiguration.getNuovo().get(_language);
			...
			for (Iterator<LanguageFieldConfigurationClassDescritor> it = lista.iterator(); it.hasNext();) {
				...
				String attribute_method = "set"
						+ tmp.getAttributeName();
				...
				if (hasAudio) {
					String audio_method = "set"
								+sub.getAudioAttribute();

					String audio_directory = "set"
								+sub.getAudioPath()	;
	...
	private Vector<String> readLabel(String _type , String label, Boolean isLinkID,
			Boolean loadaudio, Boolean primaryKey)
		...
		try {
			...
			while (field.hasNext()) {
				...
				if (primaryKey) {
					...
					// resizing keys and data arrays to the size of primary keys read
					int resizeLenght = data.size();
					String newKeysArray[] = new String[resizeLenght];
					System.arraycopy(this.key_array, 0, newKeysArray, 0, resizeLenght);
					this.key_array = newKeysArray;
					
					SearchableObject newDataArray[] =  (SearchableObject[])Array.newInstance(IClass, resizeLenght);
					 System.arraycopy(this.class_array, 0, newDataArray, 0, resizeLenght);
					 this.class_array = newDataArray;
					 log.debug("keys and data class array resized to "+resizeLenght);
				...
				if (isLinkID) {
					...
					Method2Call.invoke(class_array[counter], (Object) label+_type,
							(Object) value);
