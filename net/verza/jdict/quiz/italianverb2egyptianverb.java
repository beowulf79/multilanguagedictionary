 ...
 * 
 */
public class italianverb2egyptianverb extends QuizAbstract {

	private static Logger log;
	private Vector<Verb> localKeyArray;
	private Vector<ArabVerb> localDataArray;
	 ...
	 */
	public italianverb2egyptianverb() throws DatabaseException,
			FileNotFoundException, UnsupportedEncodingException,
			DynamicCursorException, DataNotFoundException {

		log = Logger.getLogger("net.verza.jdict.quiz");
		log.trace("called class " + this.getClass().getName());
		this.sourceLanguage = "italianverb";
		this.targetLanguage = "egyptianverb";
	...
	public void load() throws UnsupportedEncodingException, DatabaseException,
		...
		if (dbsize == 0) // if db size is 0 let's throw an exception key not
							// found
		...
		while ((max_loop_counter++ < Configuration.QUIZMAXLOOPS)
			...
			localDataArray = (Vector<ArabVerb>) dit.read("italianverb",
					key.getid().toString(), "egyptianverb").clone();

			localDataArray.iterator();
			String answer = new String();
			for (int i = 0; i < localDataArray.size(); i++) {
				answer = answer
						.concat((localDataArray.get(i).getinfinitive() == null) ? localDataArray
								.get(i).getinfinitive()
								+ " / "
								: localDataArray.get(i).getinfinitive() + " / "
										+ localDataArray.get(i).getpast())
						+ " / ";
			}
