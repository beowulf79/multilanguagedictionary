...
public final class SleepyConnector implements Dictionary  {
	 ...
	 * 
	 */
	public void setSearchKey(String indexName,
		...
		reader.setSecondaryCursor(indexName,
									db_entry);
	}
	 ...
	 */
	public Vector<SearchableObject> read(String language) throws DatabaseException,
	...
	public SearchableObject read(String _language, String _key) throws DatabaseException,
	UnsupportedEncodingException, DynamicCursorException, DataNotFoundException, KeyNotFoundException {
	...
	LanguageConfigurationClassDescriptor languageConf = 
		LanguagesConfiguration.getLanguageMainConfigNode(_language);
	List<LanguageFieldConfigurationClassDescritor> languageFieldsConf = languageConf.getFields();
	int count=0;
		for (Iterator<LanguageFieldConfigurationClassDescritor> it = languageFieldsConf.iterator(); it.hasNext();) {
			...
			if("subindex".equals(tmp.getKey_type()))	{
				reader.setDataBinding(SleepyBinding.getDataBinding());
				reader.setSecondaryCursor(SleepyFactory.getInstance().getDatabase(_language).getIndex(tmp.getAttributeName()), 
										  new DatabaseEntry(_key.getBytes("UTF-8"))  );
				count = reader.read	();
				System.out.println("entry found with index" +tmp.getAttributeName()
									+" "+count);
				if( count > 0) break;
	...
	if( count > 0) return reader.getData(0);
	 ...
	 */
	public Vector<SearchableObject> read(String _srcLang, String _key, String _dstLang) throws DatabaseException,
			UnsupportedEncodingException, DynamicCursorException, DataNotFoundException, KeyNotFoundException {
		
		SearchableObject obj = null;
		Vector<SearchableObject> linkedObjs = null;
		if((obj = read(_srcLang, _key)) == null) return null;
		
		SleepyLinkidResolver linkidResolver = new SleepyLinkidResolver();
	    linkidResolver.setSearchableObject(obj);
        linkidResolver.setLanguage(_dstLang);
	...
	/*public Vector<SearchableObject> getKey() throws DataNotFoundException {

		return reader.get.getKey();
	}*/

	/*public SearchableObject getKey(int index) throws DataNotFoundException {
	...
	public int writeData(SearchableObject w, SearchableObject d) throws DatabaseException {
	...
	public SleepyDatabase getSleepyDatabase() {
		return db;
	}
	...
	public void loadDatabase(String path) throws LabelNotFoundException,
			DatabaseException, IOException, BiffException, KeyNotFoundException {
