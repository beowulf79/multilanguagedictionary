 ...
 * 
 */
public class Audio2ArabicQuiz extends QuizInterface {
	 ...
	 */
	public Audio2ArabicQuiz() throws DatabaseException,
	...
	public int load() throws UnsupportedEncodingException, DatabaseException,
		...
		while (counter < iterations) {
			...
			Word data = localDataArray.get(number);

			log.trace("setting quiztype to AUDIO2ARABIC" + " Word_ID "
					+ data.getid() + " audio stream size " + data.getaudio());
			quizResult.setQuizType(Configuration.AUDIO2ARABIC);
			quizResult.setWord_IID(data.getid());
			// The Question String is composed by the audio object
			quizResult.setQuestion(data.getaudio());
