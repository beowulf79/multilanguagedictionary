 ...
 * 
 */
public class Italian2ArabicQuiz extends QuizInterface {
	 ...
	 */
	public Italian2ArabicQuiz() throws DatabaseException,
	...
	public int load() throws UnsupportedEncodingException, DatabaseException,
		...
		while (counter < iterations) {
			...
			log.trace("data vector size inside loop " + localDataArray.size());
			IWord data = localDataArray.get(number);

			log.trace("setting quiztype to ITALIAN2ARABIC " + " Word_ID "
					+ key.getid() + " Question " + key.getsingular());
			...
			String answer = data.getsingular();

			// Setting the correct answer by querying the linkedID
			String[] src_linkID = key.getlinkid();
			log.trace("searching iterating through linkIDs");
			if (src_linkID != null) {
				...
				for (int i = 0; i < src_linkID.length; i++) {
					...
					Vector<Word> vc = dit.getData();
					w = (IWord) vc.get(0);
				//	w = (Word) dit.getData(0); sostituire le due righe sopra con questa.
