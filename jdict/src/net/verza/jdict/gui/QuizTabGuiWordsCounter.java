...
/**
 * This class receives a string array of section or category and for each value
 * counts in the database how many words.
 * 
 * It builds a string array which is the copy of the one received adding besides
 * each value the number of the words in the database.
 * 
 * 
...
public class QuizTabGuiWordsCounter {
	...
	log.trace("called function setInputData with an array having lenght"
		+ _inp.length);
	this.input = _inp;
    ...
    public String[] search(String type) throws DatabaseException,
	    DynamicCursorException, DataNotFoundException,
	    UnsupportedEncodingException, FileNotFoundException,
	    KeyNotFoundException, SecurityException, IllegalArgumentException,
	    ClassNotFoundException, NoSuchMethodException,
	    InstantiationException, IllegalAccessException,
	    InvocationTargetException, LinkIDException,
	...
	for (int i = 0; i < this.input.length; i++) {
	    ...
	    this.output[i] = new String(this.input[i])
		    .concat("(" + count + ")");
