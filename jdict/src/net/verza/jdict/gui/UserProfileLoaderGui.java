...
public class UserProfileLoaderGui extends JPanel implements ActionListener,
	...
	jpnl1.setBorder(new EmptyBorder(new Insets(5, 5, 5, 5)));// 5
	// pixels
	// gap to
	// the
	// borders
	...
	try {
	    // set the user profile in the dictionary for this
	    // session
	...
	try {
		...
		// Password OK let's open the main Frame from here
		log.info("successfull authentication");
		// destroy and open a new instance of Window
		Window.getInstance().close();
		Window.getInstance().showGui();
		// let's delete this instance so that other
		// calls to this class will reopen the frame
		destroyInstance();
		// close the Window
		frame.dispose();
	    }
